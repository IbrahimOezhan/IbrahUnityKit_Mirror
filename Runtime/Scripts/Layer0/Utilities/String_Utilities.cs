...
namespace IbrahKit.Utilities
{
    public static class String_Utilities
    {
        ...
        {
            string finalString = list.Aggregate(string.Empty,
                (current, next) => current + (current == string.Empty ? "" : ", ") + next);
        ...
        {
            return string.IsNullOrEmpty(value) || string.IsNullOrWhiteSpace(value) || value == string.Empty ||
